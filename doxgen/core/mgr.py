...
def try_to_call(t, f, v):
    """
    Try to call function f of plugin t[] with value v
    :param t: ...
    :param f: ...
    :param v: ...
    """
...
def __load_plugins(plugins_dir: pathlib.Path) -> list:
    """
    Load all Python plugins from a directory into a dict.

    :param plugins_dir: the full path of plugins.
    :returns: pair of plugin full path and its 'main' module.
    :rtype: :class:`dict`
    :todo: replace plugin full path with end dir (like 1, 2, etc); then concat with settings
    """
    mods = list()   # [(plugindir:str (full path), module:str (like 'plugins.4.main')),]
...
def autostart(plugins_path: pathlib.Path, formgen, formsetgen) -> None:
    """
    ...
    # 2. repack
    for dir_name, module in __load_plugins(plugins_path):  # repack module objects
        # s = set(dir(module))
        data = module.DATA
